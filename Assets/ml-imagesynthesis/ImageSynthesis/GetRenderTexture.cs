...
public class GetRenderTexture : MonoBehaviour {
	...
	private void Start()
	{
		// cloneHand = GameObject.Find("LeftHandModel(Clone)");
		// var handMpb = new MaterialPropertyBlock();
		// Renderer handRender = cloneHand.GetComponentInChildren<Renderer>();
		// handMpb.SetColor("_TagColor", ColorEncoding.EncodeTagAsColor("VR Left Hand"));
		// handRender.SetPropertyBlock(handMpb);
		OnSceneChange();
	}
	...
	private void GetTexture(Camera cam,string filename, int width, int height, bool supportsAntialiasing, bool needsRescale)
	{
		var mainCamera = GetComponent<Camera>();
		var depth = 24;
		var format = RenderTextureFormat.Default;
	...
	private bool DetectPotentialSceneChangeInEditor()
	{
		...
		else if (UnityEditor.Selection.activeGameObject)
		{
			...
			var potentialChangeHappened = lastSelectedGOLayer != go.layer || !go.CompareTag(lastSelectedGOTag);
